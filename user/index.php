	...
	<form method="POST">
		<div class="container">
			...
			<header>
				...
				<h1> <strong>Secure</strong> Ministry</h1>
			...
			<section class="main">
				<div class="form-2">
					...
					$sql = "SELECT `id`, `custId`, `Message`, `Date`,status FROM `Data` WHERE `custId`=$uid";
...
if($status==1)
{
     echo "<td>  $Message  </td> <td> <button disabled type='submit' value='$id' name='decodebtn'> DECODE  </button>   </td>  ";
}
else {
	echo
								"<td> Message Encrypted </td> <td> <button type='submit' value='$id' name='decodebtn'> DECODE  </button> </td>
...
{
	$ValueId= $_POST['decodebtn'];
	$_SESSION['valiId']=$ValueId;
	echo $ValueId;
						...
						if ($imei == '0') {
							...
							$newValueId= str_pad($ValueId, 4, "0", STR_PAD_LEFT);  // produces "0001"

							$dataqr = $newValueId . $imei;
